...
class PlanarGraph(private val mapData: String,val zoomLevel : Double = 16.0){
    ...
    var mapCenter: LatLng = LatLng(30.549363171302904,114.33735709596714)
    val AUSTRALIA_BOUNDS = LatLngBounds.Builder()
            .include(LatLng(30.549363171302904 - 0.0015, 114.33735709596714 + 0.002))
            .include(LatLng(30.549363171302904 + 0.0025, 114.33735709596714 - 0.002))
            .build()
            ...
            mapDataJson.keys().forEach { key ->
                try {
                    ...
                    if ("Frame" == key) {
                        ...
                        mapCenter = LatLng(30.549363171302904,114.33735709596714)
                    }
                } catch (e: Exception) {
                    e.printStackTrace()
                }
